...
DELTA_RANGE_PITCH = 60 # Ex: +/- 45 graus para movimento de cima-baixo

# Suavização do ângulo do servo (para reduzir tremores)
ALPHA_SMOOTHING = 0.98 # Ajusta este valor experimentalmente
smoothed_pitch_angle_servo = 90 # Ângulo inicial do servo (pode ser ajustado conforme necessário)
...
def extract_gyro(data_bytes, i):
    ...
    gy = struct.unpack("<f", data_bytes[offset+16:offset+20])[0] # Apenas GY é usado aqui, mas pode mudar
...
def map_angle(value, min_val, max_val):
    ...
    A direção (0,180 ou 180,0) pode ser ajustada aqui.
    """
...
def process_imu_packet(data_bytes):
    ...
    gx, gy, gz = extract_gyro(data_bytes, sample_index_for_control)

    # =====================================================================
    # --- DEPURAR EIXOS: DESCOMENTE AS LINHAS ABAIXO PARA VER OS VALORES BRUTOS ---
    # print(f"Acel: X={ax:.2f}, Y={ay:.2f}, Z={az:.2f} | Gyro: X={gx:.2f}, Y={gy:.2f}, Z={gz:.2f}")
    # --- REMOVA ESTAS LINHAS DEPOIS DE ENCONTRAR OS EIXOS CORRETOS! ---

    # =====================================================================
    # Cálculo do Ângulo de Pitch (Estimado pelo Filtro Complementar)
    # IMPORTANTE: As linhas abaixo definem como o Pitch é calculado.
    # Ajuste 'ax', 'ay', 'az' e 'gx', 'gy', 'gz' com base na sua experimentação para mimetizar o braço.
    # Exemplo atual (provavelmente para pulso/mão):
    pitch_accel_rad = math.atan2(ax, az) # Se o eixo 'ax' e 'az' forem os que giram para o Pitch desejado.
    pitch_accel_deg = math.degrees(pitch_accel_rad)

    # Use o eixo do giroscópio que corresponde à rotação do Pitch que pretende mimetizar.
    # Mudança para 'gx' 
